...
class DataLoader(object):
    ...
    def load(self, normalize=False):
        ...
        if self.source == 'bank':
            ...
            self.data = pd.read_csv('/Users/ankitasaha/Downloads/Fair-Clustering Codes/Fair Clustering Final/data/Adult/adult2.csv')  
    ...
    def split(self, split_size, random_state):
        ...
        """
        majority_sample = self.data[self.data[self.fair_column] == 1].sample(max(split_size), random_state=random_state)
        minority_sample = self.data[self.data[self.fair_column] == 0].sample(min(split_size), random_state=random_state)
        
        df = pd.concat([majority_sample, minority_sample], ignore_index=True)
